...
@Component({
  selector: 'app-all-port',
  templateUrl: './all-port.component.html',
  styleUrl: './all-port.component.scss'
})
export class AllPortComponent extends DataTableSelectionBase(adminBaseMixin(Component)) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm') private searchForm: NgForm;

  // Store Data
  override allTableData: Port[] = [];
  private holdPrevUsers: Port[] = [];
  ...
  private readonly portDataService = inject(PortService);
  ...
  ngAfterViewInit(): void {
    ...
    this.subSearch = formValue.pipe(
      ...
      switchMap(data => {
        ...
        return this.portDataService.getAllPort(filterData, this.searchQuery);
      })
    ).subscribe({
      next: res => {
  ...
  private getAllUsers() {
    ...
    this.subDataGetAll = this.portDataService.getAllPort(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.portDataService.deleteMultiplePortById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCategoryById(data: any) {
    this.subDataUpdateMulti = this.portDataService.updateMultiplePortById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private addManyPort(data: any[]) {
    // Start Request Time
    this.reqStartTime = new Date();

    this.subDataAddMulti = this.portDataService.addManyPort(data)
      .subscribe({
        next: res => {
          this.uiService.message(res.message, 'success');
          this.isLoading = false;
