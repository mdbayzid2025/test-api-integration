...
export class AllCategoryComponent extends DataTableSelectionBase(adminBaseMixin(Component)) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm') private searchForm: NgForm;

  // Store Data
  override allTableData: ThemeCategory[] = [];
  private holdPrevUsers: ThemeCategory[] = [];
  ...
  private readonly categoryDataService = inject(ThemeCategoryService);
  ...
  ngAfterViewInit(): void {
    ...
    this.subSearch = formValue.pipe(
      ...
      switchMap(data => {
        ...
        return this.categoryDataService.getAllCategories(filterData, this.searchQuery);
      })
    ).subscribe({
      next: res => {
  ...
  private getAllUsers() {
    ...
    this.subDataGetAll = this.categoryDataService.getAllCategories(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.categoryDataService.deleteMultipleThemeCategoryById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCategoryById(data: any) {
    this.subDataUpdateMulti = this.categoryDataService.updateMultipleThemeCategoryById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
