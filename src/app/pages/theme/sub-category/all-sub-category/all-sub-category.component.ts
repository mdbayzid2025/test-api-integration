...
export class AllSubCategoryComponent extends DataTableSelectionBase(adminBaseMixin(Component)) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm') private searchForm: NgForm;

  // Store Data
  override allTableData: ThemeSubCategory[] = [];
  private holdPrevUsers: ThemeSubCategory[] = [];
  ...
  ngAfterViewInit(): void {
    ...
    this.subSearch = formValue.pipe(
      ...
      switchMap(data => {
        ...
        return this.subCategoryService.getAllThemeSubCategories(filterData, this.searchQuery);
      })
    ).subscribe({
      next: res => {
  ...
  private getAllUsers() {
    ...
    this.subDataGetAll = this.subCategoryService.getAllThemeSubCategories(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.subCategoryService.deleteMultipleThemeSubCategoryById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCategoryById(data: any) {
    this.subDataUpdateMulti = this.subCategoryService.updateMultipleThemeSubCategoryById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
