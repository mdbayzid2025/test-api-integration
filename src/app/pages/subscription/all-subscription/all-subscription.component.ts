...
import {EMPTY, filter, map, Subscription, switchMap} from "rxjs";
...
export class AllSubscriptionComponent extends DataTableSelectionBase(adminBaseMixin(Component)) implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    this.subSearch = formValue.pipe(
      ...
      switchMap(data => {
        ...
        return this.subscriptionDataService.getAllSubscription(filterData, this.searchQuery);
      })
    ).subscribe({
      next: res => {
  ...
  private getAllSubscriptions() {
    ...
    this.subDataGetAll = this.subscriptionDataService.getAllSubscription(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.subscriptionDataService.deleteMultipleSubscriptionById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
