...
import {PaymentLinkHistoryService} from "../../../services/common/payment-link-history.service";


@Component({
  selector: 'app-all-payment-link-history',
  templateUrl: './all-payment-link-history.component.html',
  styleUrl: './all-payment-link-history.component.scss'
})
export class AllPaymentLinkHistoryComponent extends adminBaseMixin(Component) implements AfterViewInit, OnDestroy {

  // Store Data
  paymentLinkHistorys: PaymentLinkHistory[] = [];
  private holdPrevUsers: PaymentLinkHistory[] = [];
  ...
  navArray: NavBreadcrumb[] = [
    ...
    {name: 'All Payment Link History', url: null},
  ];
  ...
  private readonly paymentLinkHistoryDataService = inject(PaymentLinkHistoryService);
  private readonly clipboard = inject(Clipboard);

  ngOnInit() {
    // Reload Data
    this.subReload = this.reloadService.refreshData$.subscribe(() => {
      this.getAllPaymentLinkHistorys();
    })
    ...
    this.subSearch = formValue.pipe(
      ...
      switchMap(data => {
        ...
        if (this.searchQuery === '' || this.searchQuery === null) {
          this.searchUsers = [];
          this.paymentLinkHistorys = this.holdPrevUsers;
        ...
        return this.paymentLinkHistoryDataService.getAllPaymentLinkHistorys(filterData, this.searchQuery);
      })
    ).subscribe({
      next: res => {
        this.searchUsers = res.data;
        this.paymentLinkHistorys = this.searchUsers;
   ...
   */
  private getAllPaymentLinkHistorys() {
    const pagination: Pagination = {
      pageSize: Number(this.dataPerPage),
      currentPage: Number(this.currentPage) - 1
    };
    ...
    this.subDataGetAll = this.paymentLinkHistoryDataService.getAllPaymentLinkHistorys(filterData, this.searchQuery)
      .subscribe({
        next: res => {
          this.isLoading = false;
          this.paymentLinkHistorys = res.data;
          if (this.paymentLinkHistorys && this.paymentLinkHistorys.length) {
            this.paymentLinkHistorys.forEach((m, i) => {
              const index = this.selectedIds.findIndex(f => f === m._id);
              this.paymentLinkHistorys[i].select = index !== -1;
            });
  ...
  private updateMultipleUserById(data: any) {
    this.subDataUpdateMulti = this.paymentLinkHistoryDataService.updateMultiplePaymentLinkHistoryById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.paymentLinkHistoryDataService.deleteMultiplePaymentLinkHistoryById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private checkSelectionData() {
    ...
    this.paymentLinkHistorys.forEach(m => {
      if (!m.select) {
        isAllSelect = false;
      }
    });
  ...
  onAllSelectChange(event: MatCheckboxChange) {
    const currentPageIds = this.paymentLinkHistorys.map(m => m._id);
    if (event.checked) {
      ...
      currentPageIds.forEach(m => {
        this.paymentLinkHistorys.find(f => f._id === m).select = false;
  ...
  copyToClipboard($event: Event, paymentLinkHistoryId: string): void {
    $event.stopPropagation();

    const fullUrl = `${this.baseLink}/payment/${paymentLinkHistoryId}`;
