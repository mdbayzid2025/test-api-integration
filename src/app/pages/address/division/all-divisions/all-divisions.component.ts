...
export class AllDivisionsComponent extends adminBaseMixin(Component) implements AfterViewInit, OnDestroy {

  // Store Data
  divisions: Division[] = [];
  private holdPrevUsers: Division[] = [];
    ...
    this.subSearch = formValue.pipe(
      ...
      switchMap(data => {
        ...
        if (this.searchQuery === '' || this.searchQuery === null) {
          this.searchUsers = [];
          this.divisions = this.holdPrevUsers;
        ...
        return this.divisionDataService.getAllDivisions(filterData, this.searchQuery);
      })
    ).subscribe({
      next: res => {
        this.searchUsers = res.data;
        this.divisions = this.searchUsers;
  ...
  private getAllDivisions() {
    ...
    this.subDataGetAll = this.divisionDataService.getAllDivisions(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.divisionDataService.deleteMultipleDivisionById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleUserById(data: any) {
    this.subDataUpdateMulti = this.divisionDataService.updateMultipleDivisionById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
