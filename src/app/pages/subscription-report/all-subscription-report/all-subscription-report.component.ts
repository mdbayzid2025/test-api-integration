...
@Component({
  selector: 'app-all-subscription-report',
  templateUrl: './all-subscription-report.component.html',
  styleUrl: './all-subscription-report.component.scss'
})
export class AllSubscriptionReportComponent  extends DataTableSelectionBase(adminBaseMixin(Component)) implements AfterViewInit, OnDestroy {
  ...
  private getAllSubscriptions() {
    ...
    const filterData: FilterData = {
      pagination: pagination,
      filter: {...this.filter, ...{ paymentStatus: { $nin: ['unpaid', 'paid'] }}},
      select: this.select,
      sort: this.sortQuery
    }
