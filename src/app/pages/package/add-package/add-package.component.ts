...
export class AddPackageComponent extends adminBaseMixin(Component) implements OnInit, OnDestroy {
  ...
  private initDataForm() {
    ...
    this.priceTypeDataArray = this.dataForm.get(
      'priceTypes'
    ) as FormArray;
  }
  ...
  private setFormValue() {
    ...
    if (this.package.image) {
      this.pickedImages = [this.package.image];
    }
    // Form Array
    ...
    if (this.package.priceTypes && this.package.priceTypes.length) {
      this.package.priceTypes.map((m) => {
        const f = this.fb.group({
          name: [m.name],
          value: [m.value],
        });
        (this.dataForm?.get('priceTypes') as FormArray).push(f);
      });
    }
  ...
  removeFormArrayField(formControl: string, index: number) {
    ...
    switch (formControl) {
      ...
      case 'priceTypes': {
        formDataArray = this.priceTypeDataArray;
        break;
      }
      case 'routeLimits': {
